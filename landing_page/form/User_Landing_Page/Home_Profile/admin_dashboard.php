...
if ($result->num_rows > 0) {
    ...
    $profile_pic = $admin['profile_pic'] ?? 'images/PESO-Logo-Assets.png';
} else {
    session_unset();
    session_destroy();
    header("Location: ../../signin.php");
...
";
    ...
    if (isset($_POST['add_scholarship'])) {
        ...
        // status changed from 'pending' to 'active' for automatic publishing
        $insertSql = "INSERT INTO scholarships (title, description, requirements, benefits, eligibility, number_of_slots, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'active', NOW())";
        $insertStmt = $conn->prepare($insertSql);
        $insertStmt->bind_param("sssssi", $title, $description, $requirements, $benefits, $eligibility, $slots);

        if ($insertStmt->execute()) {
            $_SESSION['scholarship_published'] = true; // Using the published session for the toast notification
        }
...
if (isset($_POST['approve_user'])) {
    ...
    $result = $conn->query("SELECT Email, Fname, Lname, status FROM user WHERE user_id=$uid");
...
$scholarshipDocsSql .= " ORDER BY s.title ASC, u.Lname ASC";
...
foreach ($scholarship_docs_users as $user) {
    ...
    if (!isset($scholarshipReportsData[$sch_id])) {
        $scholarshipReportsData[$sch_id] = [
            'title' => $user['title'],
            'users' => []
        ];
    }
...
<style>
    ...
    .concerns-chat-list li:hover,
    .concerns-chat-list li.active {
        background: #e0e7ff;
        font-weight: bold;
    }
    ...
    .modal-header {
        background: linear-gradient(135deg, #090549 0%, #10087c 100%);
        color: white;
        padding: 20px 25px;
    ...
    .modal-header i {
        font-size: 24px;
        color: #ffcc00;
        /* Gold accent for the icon */
    }

    #appFormModal .modal-body,
    #spesAppModal .modal-body {
        max-height: 400px;
        overflow-y: auto;
    }
...
<body>
    ...
    <div class="container">
        ...
        <?php unset($_SESSION['scholarship_added']);
        endif; ?>
        ...
        <?php unset($_SESSION['scholarship_ended']);
        endif; ?>
        ...
        <?php unset($_SESSION['scholarship_published']);
        endif; ?>
        ...
        <?php unset($_SESSION['slots_updated']);
        endif; ?>
        ...
        <?php unset($_SESSION['message_deleted']);
        endif; ?>
        ...
        <?php unset($_SESSION['document_saved_success']);
        endif; ?>
        ...
        <?php unset($_SESSION['document_saved_error']);
        endif; ?>
        ...
        <?php unset($_SESSION['spes_batch_success']);
        endif; ?>
        ...
        <?php unset($_SESSION['spes_batch_error']);
        endif; ?>
        ...
        <?php unset($_SESSION['spes_form_upload_success']);
        endif; ?>
        ...
        <?php unset($_SESSION['spes_form_upload_error']);
        endif; ?>
        ...
        <?php if (isset($_SESSION['user_request_success'])): ?>
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    showToast('<?php echo addslashes($_SESSION['user_request_success']); ?>', 'success');
                });
            </script>
        <?php unset($_SESSION['user_request_success']);
        endif; ?>

        <?php if (isset($_SESSION['user_request_error'])): ?>
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    showToast('<?php echo addslashes($_SESSION['user_request_error']); ?>', 'error');
                });
            </script>
        <?php unset($_SESSION['user_request_error']);
        endif; ?>
        ...
        <?php unset($_SESSION['scholarship_deleted_permanently']);
        endif; ?>
        ...
        <div class="main-content">
                    ...
                    <div class="applicants-container">
                        ...
                        <table class="applicants-table">
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center" style="padding: 20px;">No applicants found for this scholarship or search query.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
            ...
            <div id="total-applicants-spes" class="page">
                <div class="applicants-container">
                    ...
                    $spesListSql = "
                        ...
                        FROM spes_applications sa
                        JOIN user u ON sa.user_id = u.user_id
                        LEFT JOIN spes_batches sb ON sa.created_at >= sb.start_date AND (sa.created_at <= sb.end_date OR sb.end_date IS NULL)
                    ";
                    ...
                    <table class="applicants-table">
                        ...
                        <tbody>
                            <?php if (count($spesApplicantsList) > 0): ?>
                                <?php foreach ($spesApplicantsList as $app): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($app['Fname'] . ' ' . $app['Lname']); ?></td>
                                        <td><strong><?php echo htmlspecialchars($app['batch_name'] ?? 'N/A'); ?></strong></td>
                                        <td><?php echo date('M d, Y', strtotime($app['created_at'])); ?></td>
                                        <td>
                                            <?php
                                            $statusClass = 'status-' . htmlspecialchars($app['status']);
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="text-center" style="padding: 20px;">No SPES applicants found for this search.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                ...
                <div class="applicants-container">
                    ...
                    <table class="applicants-table">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="text-center" style="padding: 20px;">No approved SPES applicants found for this search.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
            ...
            <div id="spes-document-page" class="page">
                <div class="applicants-container">
                    ...
                    <button class="back-btn" onclick="showPage('reports-page')">Back to Reports</button>
                    <a href="generate_pdf_report.php?report=spes" class="btn-download" target="_blank" style="text-decoration:none; display:inline-block; vertical-align:middle;">
                    ...
                    <table class="applicants-table">
                        ...
                        <tbody>
                            <?php foreach ($spes_docs_users as $user): ?>
                                ...
                                <tr>
                                    <td>
                                        <button class="btn-outline" <?php if (!$has_id && !$has_reqs) echo 'disabled'; ?>
                                            onclick='viewSpesUserDocuments(<?php echo json_encode(["ids" => json_decode($user["id_image_paths"]), "reqs" => $user["spes_documents_path"]]); ?>, "<?php echo htmlspecialchars($user['Fname'] . ' ' . $user['Lname']); ?>")'>
                                            View Documents
                                        </button>
                                    </td>
                                    <td><?php echo htmlspecialchars($user['Fname'] . ' ' . $user['Lname']); ?></td>
                            ...
                            <?php if (count($spes_docs_users) === 0): ?>
                                <tr>
                                    <td colspan="5" class="text-center" style="padding: 20px;">No approved SPES applicants found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                            ...
                            <?php if ($selectedGroupId || $selectedUserId): ?>
                                ...
                                if (!empty($userIdsInChat)) {
                                    ...
                                    if (!empty($userIdsInChat)) {
                                        ...
                                        $result = $stmt->get_result();
                                        while ($row = $result->fetch_assoc()) {
                                            $approvedScholarUsers[] = $row['user_id'];
                                        }
                        ...
                        <div class="scholarship-accordion">
                            <button class="scholarship-accordion-header">
                                ...
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    <span class="scholarship-status <?php echo $scholarship['status'] === 'active' ? 'status-active' : 'status-pending'; ?>" style="font-size: 10px;">
                ...
                <div id="SpesMgmt" class="tab-content">
                    ...
                    <table class="applicants-table">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="text-center" style="padding: 20px;">No SPES batches found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
            ...
            <div id="scholarship-history-page" class="page">
                <h1 class="main-title-scholar">Scholarship & SPES History</h1>
                ...
                <div class="tabs">
                    ...
                    <button class="tab-link" onclick="openHistoryTab(event, 'EndedScholarshipsTab')">Scholarships History (Closed)</button>
                            ...
                            <?php if (count($activeScholarships) > 0): ?>
                                <?php foreach ($activeScholarships as $scholarship): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="?view_recent_members=<?php echo $scholarship['scholarship_id']; ?>#view-recent-members-page" class="view-details"><i class="fas fa-eye"></i> View Members</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="text-center" style="padding: 20px;">No recent (active) scholarships found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                            ...
                            <?php if (count($endedScholarships) > 0): ?>
                                <?php foreach ($endedScholarships as $scholarship): ?>
                                    <tr>
                                        ...
                                        <td><?php echo date('F d, Y', strtotime($scholarship['created_at'])); ?></td>
                                        <td>
                                            <a href="?view_ended_members=<?php echo $scholarship['scholarship_id']; ?>#view-ended-members-page" class="view-details"><i class="fas fa-eye"></i> View Members</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="text-center" style="padding: 20px;">No ended scholarships found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                            ...
                            <?php if (count($spesHistory) > 0): ?>
                                <?php foreach ($spesHistory as $batch): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="?view_spes_members=<?php echo $batch['batch_id']; ?>#view-spes-members-page" class="view-details"><i class="fas fa-eye"></i> View Members</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="text-center" style="padding: 20px;">No SPES History found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                    ...
                    <table class="applicants-table">
                        <thead>
                            <tr>
                                <th>Name</th>
                                <th>Program</th>
                                <th>Scholarship Application Date</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="3" class="text-center">No members found for this scholarship.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                ...
                <div class="applicants-container">
                    ...
                    <table class="applicants-table">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="3" class="text-center">No members found for this ended scholarship.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                ...
                <div class="applicants-container">
                    ...
                    <table class="applicants-table">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="3" class="text-center">No members found for this SPES batch.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
...
Closing/Signature:" rows="5" required></textarea>
    ...
    <script>
        ...
        function showAppFormModal(applicantJson) {
            ...
            <span>Scholarship Application Details</span>
        `;

            document.querySelector('#appFormModal .modal-header').innerHTML = headerHTML;

            let html = '<div style="font-size: 13px; line-height: 1.6; color: #333;">';
            ...
            <div class="user-details-item" style="margin-bottom: 8px;">
                <span class="user-details-label" style="font-weight: 600; color: #090549; display: block; font-size: 11px; text-transform: uppercase;">${label}</span>
                <div class="user-details-value" style="font-size: 14px; border-bottom: 1px solid #eee; padding-bottom: 4px;">${value || 'N/A'}</div>
            </div>`;
            ...
            html += `<table class="applicants-table" style="font-size: 11px;">
                ...
                <tbody>
                    <tr><td>Elementary</td><td>${app.elem_school || 'N/A'}</td><td>${app.elem_honors || 'N/A'}</td><td>${app.elem_grad || 'N/A'}</td></tr>
                    <tr><td>High School</td><td>${app.hs_school || 'N/A'}</td><td>${app.hs_honors || 'N/A'}</td><td>${app.hs_grad || 'N/A'}</td></tr>
                    <tr><td>Vocational</td><td>${app.voc_school || 'N/A'}</td><td>${app.voc_honors || 'N/A'}</td><td>${app.voc_grad || 'N/A'}</td></tr>
                </tbody>
            </table>`;
    ...
    `;
        ...
        function showSpesAppModal(spesAppJson) {
            ...
            const createDetailItem = (label, value) => `<div class="user-details-item"><span class="user-details-label">${label}</span><div class="user-details-value">${value || 'N/A'}</div></div>`;
            ...
            html += `</div>`;
            html += `<div class="user-details-item" style="margin-top:15px;"><span class="user-details-label">Present Address</span><div class="user-details-value">${app.present_address || 'N/A'}</div></div>`;
            html += `<div class="user-details-item" style="margin-top:15px;"><span class="user-details-label">Permanent Address</span><div class="user-details-value">${app.permanent_address || 'N/A'}</div></div>`;

            html += '<div class="user-details-section-header" style="margin-top: 20px;">2. Parental & Status Information</div>';
            html += '<div class="user-details-grid">';
            html += createDetailItem('GSIS Beneficiary', app.gsis_beneficiary);
            html += createDetailItem('Student Type', app.student_type);
            html += createDetailItem("Father's Info", `${app.father_name_contact || 'N/A'} - <i>${app.father_occupation || 'N/A'}</i>`);
            html += createDetailItem("Mother's Info", `${app.mother_name_contact || 'N/A'} - <i>${app.mother_occupation || 'N/A'}</i>`);
            html += `</div>`;
            html += `<div class="user-details-item" style="margin-top:15px;"><span class="user-details-label">Parent Status</span><div class="user-details-value">${app.parent_status || 'N/A'}</div></div>`;
            ...
            html += '<div class="user-details-section-header" style="margin-top: 15px;">4. Skills & SPES History</div>';
            html += '<div class="user-details-grid">';
            ...
            html += '<div class="user-details-section-header" style="margin-top: 20px;">5. Uploaded Documents</div>';
            html += `<h4>Uploaded ID(s):</h4>`;
            try {
                ...
                if (Array.isArray(idImagePaths) && idImagePaths.length > 0 && idImagePaths[0]) {
                    idImagePaths.forEach(path => {
                        ...
                        html += `<a href="${fullPath}" target="_blank" style="margin-right: 10px;"><img src="${fullPath}" alt="ID" style="max-width: 200px; height: auto; border-radius: 5px;"></a>`;
                    });
                } else {
            ...
            html += `<h4 style="margin-top:15px;">Requirement Documents:</h4>`;
        ...
        function showValidIdModal(validIdJson) {
            ...
            if (Array.isArray(idFiles) && idFiles.length > 0) {
                // Display Front ID
                if (idFiles[0]) {
                    const frontPath = `<?php echo BASE_URL; ?>${idFiles[0].replace('../../../../', '').replace('E-Scholarship-SPES/', '')}`;
                ...
                // Display Back ID
                if (idFiles[1]) {
                    const backPath = `<?php echo BASE_URL; ?>${idFiles[1].replace('../../../../', '').replace('E-Scholarship-SPES/', '')}`;
